...
public class JwtFilter extends OncePerRequestFilter { // OncePerRequestFilter - Ensures this filter runs once per request, unlike generic filters.
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,@NonNull HttpServletResponse response, @NonNull FilterChain filterChain)
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {  // - Extracts the JWT token.
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (jwtUtil.validateToken(token, userDetails)) {
                ...
                SecurityContextHolder.getContext().setAuthentication(authToken);  // Sets the authentication in the security context so that it can be used later in the request processing.
            }
        }
